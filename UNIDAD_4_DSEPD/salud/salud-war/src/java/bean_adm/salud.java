...
public class salud {
   ...
   // private double cadera;
    ...
    public String getMensaje_antro_existe() {
        return mensaje_antro_existe;
    }

    public void setMensaje_antro_existe(String mensaje_antro_existe) {
        this.mensaje_antro_existe = mensaje_antro_existe;
    }
...
/*
    ...
    */
    ...
    public List<Antropometricos> lista_ant_por_idureg(){
        List<Antropometricos> ua = new ArrayList<>();
        for (Antropometricos a : antropometricos_ln.listaAntropometricos()) {
            if (ultimo == a.getIdUsu().getIdUsuario()) {
                ua.add(a);
                break;
            }
        }
...
// usuarios.setFechanac(obtenerFecha());
   ...
   public boolean es_fecha_registrada(Date fechare){
       boolean is_regis=false;
        for (int i = 0; i < lista_ant_por_idureg().size(); i++) {
            if (( lista_ant_por_idureg().get(i).getFecha()).equals(fechare)) {//fecha_reg_ant //si el usuario ya esta registrado
                //antropometricos_ulogeado = antropometricos_ln.listaAntropometricos().get(i);   
                is_regis=true;
                break;
            }
    ...
    */
    ...
    public boolean is_hombre(char sexo){
     ...
     public ArrayList<Estadisticas> calcular_estadisticas()
    {
        ArrayList<Estadisticas>est=new ArrayList<>();
        for(Antropometricos ant:lista_antro_por_idusuario())
        {
            double imcc,iccc;
            imcc = (ant.getPeso() / Math.pow(mostrar().getEstatura(), 2)) * 10000;
            iccc = ant.getCintura() / ant.getCadera();  
            Estadisticas e=new Estadisticas(imcc,iccc,ant.getFecha(),mostrar().getEstatura(),ant.getPeso(),ant.getCadera(),ant.getCintura());
            est.add(e);
            
        }
        return est;
    }
    public String mostrar_mes(Date fecha){
        String mes="";
        if(fecha.getMonth()==01){
            mes = "Enero";
        }
        if(fecha.getMonth()==02){
            mes = "Febrero";
        }
        if(fecha.getMonth()==03){
            mes = "Marzo";
        }
        if(fecha.getMonth()==04){
            mes = "Abril";
        }
        if(fecha.getMonth()==05){
            mes = "Mayo";
        }
        if(fecha.getMonth()==06){
            mes = "Junio";
        }
        if(fecha.getMonth()==07){
            mes = "Julio";
        }
    ...
    public String mostrar_fecha(Date fecha_chequeo){
        //SimpleDateFormat sdf = new SimpleDateFormat("yyyy");
        //String anio = sdf.format(fecha_chequeo);
        return mostrar_mes(fecha_chequeo); //+ "/" + anio;
    }
    public String graficar_imc(double imc){    
        int imc_redondeado = (int)imc;
        String puntos="";
        for (int i = 0; i < imc_redondeado; i++) {
            puntos += "*"; 
        }
        return puntos;
    }
