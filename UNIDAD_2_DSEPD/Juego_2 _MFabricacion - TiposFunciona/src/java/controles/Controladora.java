...
public abstract class Controladora<T> implements Serializable {
    ...
    public abstract void create(Tipos tipos) throws RollbackFailureException, Exception;
    
    public abstract void edit(Tipos tipos) throws NonexistentEntityException, RollbackFailureException, Exception;
    ...
    private List<T> findEntities(boolean all, int maxResults, int firstResult) {
        ...
        try {
            ...
            //cq.select(cq.from(modelo));
            Query q = em.createQuery("select object(o) from Tipos as o");
            /////Query q = em.createQuery("select object(o) from " + modelo + " as o");
            if (!all) {
    ...
    public int getCount() {
        ...
        try {
            /*
            CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
            Root<T> rt = cq.from(modelo);
            cq.select(em.getCriteriaBuilder().count(rt));
            Query q = em.createQuery(cq);
            */
            Query q = em.createQuery("select count(o) from Tipos as o");
           //// Query q = em.createQuery("select count(o) from " + modelo +" as o");
